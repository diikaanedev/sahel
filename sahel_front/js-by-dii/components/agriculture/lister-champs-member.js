...
export default class ListerChampsMember extends HTMLElement {

    constructor() { super() }

    static _create = () => {
        const tabs = [
            { titre: 'Matricule Membres ' }
        ]

        return AddFormHtml('Lister Champs d\'un Membre', tabs, this.listChampsMember, 'Lister');
    }

    static listChampsMember = e => {
        e.preventDefault();
        ...
        Post('membre/lister-champs-member', body).then(result => {
            console.log(result);
    ...
    static _tabsChamps = (data) => {
        ...
        _header.innerHTML = 'Liste des Champs';
    ...
    static _TrTd = (tabs) => {
        ...
        tabs.forEach(element => {
            ...
            nom.innerHTML = `${element.membres.prenom} ${element.membres.nom}`;

            numero_parcelles.innerHTML = `${element.matricule}`
            longitude.innerHTML = `${element.longitude}`;
            lattitude.innerHTML = `${element.lattitude}`;
            surface_total.innerHTML = `${element.surface_total}`;
            surface_cultive.innerHTML = `${element.surface_cultive}`;
            surface_mise_en_jachere.innerHTML = `${element.surface_mise_en_jachere}`;
