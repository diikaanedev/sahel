import { AddFormSelectMarcheHtml, div, attribute, section, header, table, tbody, tr, th, i, td } from "../../configs/html-element.js"
...
export default class ListerByCircuit extends HTMLElement {

    constructor() { super() }

    static _create = () => AddFormSelectMarcheHtml('Lister Commercialisation Par Circuit' , [] , this._listerByCircuitEvent , 'lister')

    static _listerByCircuitEvent = e => {
        
        e.preventDefault();

        let circuit =  e.target.getElementsByTagName('select')[0].value;


        Post('commercialisation/circuit' , {circuit}).then((result) => {
            console.log(result);
            
            BaseTemplate.body(Navbar._create() , Aside._create() , this._tabsChamps(result.commerce , result.operateurs , result.retcoltes , result.cultures , result.agriculteurs))
        }).catch((err) => {
            console.log(err);
    ...
    static _tabsChamps = (operationCommerciales, operateurs, recoltes, cultures , agriculteurs) => {
        ...
        _header.innerHTML = `Liste des Ventes Par Circuit`;
